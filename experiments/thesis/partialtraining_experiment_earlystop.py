            ...
            for run in range(1, config.num_runs+1):
                ...
                with model_neural_trad.TradNeuralModel(
                    ...
                    while True:
                        attempts += 1
                        model.init_params()
                        langmod_fit_stats = model.fit(dataset, config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_model.hdf5', max_batch_size=config.val_batch_size, minibatch_size=langmod_minibatch_size, max_epochs=max_epochs, early_stop_patience=config.early_stop_patience, listener=_FitListener())
                        if langmod_fit_stats['stopped_early'] == False:
                            break
                        elif attempts >= config.partialtraining_max_attempts:
                ...
                with open(config.results_dir+'/partialtraining/'+corpus+'/results1_earlystop.txt', 'a', encoding='utf-8') as f:
                    print(
                        corpus,
                        frozen_prefix,
                        max_epochs,
