...
for architecture in ['init', 'pre', 'par', 'merge']:
    ...
    if not lib.file_exists(config.results_dir+'/imageimportance/'+architecture+'/results.txt'):
        with open(config.results_dir+'/imageimportance/'+architecture+'/results.txt', 'w', encoding='utf-8') as f:
            print(
                    'architecture',
                    'dataset',
                    'run',
                    ...
                    'token_index',
                    'num_caps',
                    'gradient_wrt_image_next',
                    'gradient_wrt_image_max',
                    ...
                    'gradient_wrt_postimage_next',
                    'gradient_wrt_postimage_max',
                    'gradient_wrt_multimodalvec_next',
                    'gradient_wrt_multimodalvec_max',
                    'omission_image_cos',
                    'omission_postimage_cos',
                    'omission_multimodalvec_cos',
                    'omission_logits_cos',
                    'omission_output_cos',
                    'omission_output_jsd',
                    'num_negatives_logit',
                    'frac_negatives_logit',
                    'total_time',
                    sep='\t', file=f
                )
    already_seen = set()
    with open(config.results_dir+'/imageimportance/'+architecture+'/results.txt', 'r', encoding='utf-8') as f:
                ...
                with model:
                    for sent_len in sorted({len(sent) for sent in correct_sents}):
                        ...
                        for token_index in range(sent_len+1):
                            ...
                            [
                                grad_next_wrt_image, grad_max_wrt_image,
                                ...
                                grad_next_wrt_postimage, grad_max_wrt_postimage,
                                grad_next_wrt_multimodalvec, grad_max_wrt_multimodalvec,
                            ] = model.get_sensitivity(prefixes=filtered_sents_datasource.index_sents.prefixes, prefixes_lens=filtered_sents_datasource.index_sents.lens, images=filtered_correct_images, targets=filtered_sents_datasource.index_sents.targets, temperature=1.0)
                            mean_grad_next_wrt_image = np.mean(np.mean(grad_next_wrt_image, axis=1))
                            mean_grad_max_wrt_image = np.mean(np.mean(grad_max_wrt_image, axis=1))
                            ...
                            mean_grad_next_wrt_postimage = np.mean(np.mean(grad_next_wrt_postimage, axis=1))
                            mean_grad_max_wrt_postimage = np.mean(np.mean(grad_max_wrt_postimage, axis=1))
                            ...
                            [
                                foil_post_images,
                                foil_multimodal_vectors,
                                foil_logits,
                                foil_predictions
                            ] = model.get_hidden_layers(max_batch_size=config.val_batch_size, prefixes=filtered_sents_datasource.index_sents.prefixes, prefixes_lens=filtered_sents_datasource.index_sents.lens, images=filtered_foil_images, temperature=1.0)
                            omission_image_cos = np.mean([ distance.cosine(corr, foil) for (corr, foil) in zip(filtered_correct_images, filtered_foil_images) ])
                            omission_postimage_cos = np.mean([ distance.cosine(corr, foil) for (corr, foil) in zip(corr_post_images, foil_post_images) ])
                            omission_multimodalvec_cos = np.mean([ distance.cosine(corr, foil) for (corr, foil) in zip(corr_multimodal_vectors, foil_multimodal_vectors) ])
                            omission_logits_cos = np.mean([ distance.cosine(corr, foil) for (corr, foil) in zip(corr_logits, foil_logits) ])
                            omission_output_cos = np.mean([ distance.cosine(corr, foil) for (corr, foil) in zip(corr_predictions, foil_predictions) ])
                            omission_output_jsd = np.mean([ jsd(corr, foil) for (corr, foil) in zip(corr_predictions, foil_predictions) ])
                            
                            num_neg_logits = np.mean(np.sum(corr_logits < 0.0, axis=1))
                            frac_neg_logits = num_neg_logits/vocab.size
                            
                            full_duration = full_timer.get_duration()
                            
                            with open(config.results_dir+'/imageimportance/'+architecture+'/results.txt', 'a', encoding='utf-8') as f:
                                print(
                                    architecture,
                                    dataset_name,
                                    run,
                                    ...
                                    sent_len-1,
                                    token_index,
                                    len(filtered_correct_sents),
                                    mean_grad_next_wrt_image,
                                    mean_grad_max_wrt_image,
                                    ...
                                    mean_grad_next_wrt_postimage,
                                    mean_grad_max_wrt_postimage,
