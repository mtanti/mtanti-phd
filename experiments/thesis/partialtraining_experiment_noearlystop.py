...
for corpus in corpora:
    ...
    if not lib.file_exists(config.results_dir+'/partialtraining/'+corpus+'/results2_noearlystop.txt'):
        with open(config.results_dir+'/partialtraining/'+corpus+'/results2_noearlystop.txt', 'w', encoding='utf-8') as f:
            print(
                    'corpus',
                    'frozen_prefix',
                    'max_epochs',
    ...
    already_seen = set()
    with open(config.results_dir+'/partialtraining/'+corpus+'/results1_earlystop.txt', 'r', encoding='utf-8') as f:
    ...
    for (corpus, frozen_prefix, max_epochs, run) in already_seen:
        grouped_runs[(corpus, frozen_prefix, max_epochs)].append(run)
    found_incomplete_runs = False
    for ((corpus, frozen_prefix, max_epochs), runs) in grouped_runs.items():
        if len(runs) != config.num_runs:
            print('Incomplete runs:', corpus, frozen_prefix, max_epochs, ','.join(str(run) for run in sorted(runs)))
            found_incomplete_runs = True
    if found_incomplete_runs == True:
        print('Remove incomplete runs from results1_earlystop.txt before continueing.')
        break

    with open(config.results_dir+'/partialtraining/'+corpus+'/results2_noearlystop.txt', 'r', encoding='utf-8') as f:
        for line in f.read().strip().split('\n')[1:]:
            [
                frozen_prefix,
                max_epochs,
    ...
    corpus_size_factor_exponent = 0
    for frozen_prefix in [True, False]:
        for max_epochs in range(0, config.partialtraining_max_max_epochs+1):
            for run in range(1, config.num_runs+1):
                
                print('='*100)
                print(lib.formatted_clock())
                print(corpus, frozen_prefix, max_epochs, run)
                print()
                
                if (corpus, frozen_prefix, max_epochs, run) in already_seen:
                    print('Found ready')
                    print()
                    continue
                ...
                dir_name = '{}_{}_{}_{}'.format(corpus, frozen_prefix, max_epochs, run)
                lib.create_dir(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name)
                ...
                with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_corpus_indexes.txt', 'w', encoding='utf-8') as f:
                    print(*dataset.train.individual_indexes, sep='\n', file=f)
                
                with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_selected_test.txt', 'w', encoding='utf-8') as f:
                    print(*selected_test_sents.individual_indexes, sep='\n', file=f)
                
                langmod_vocab.save_vocab(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_vocab.json')
                ...
                with model_neural_trad.TradNeuralModel(
                    ...
                    langmod_fit_stats = model.fit(dataset, config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_model.hdf5', max_batch_size=config.val_batch_size, minibatch_size=langmod_minibatch_size, max_epochs=max_epochs, early_stop_patience=None, listener=_FitListener())
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_train.txt', 'w', encoding='utf-8') as f:
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_probs.txt', 'w', encoding='utf-8') as f:
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_sents.txt', 'w', encoding='utf-8') as f:
                ...
                prefix_params = model_neural_trad.TradNeuralModel.get_saved_prefix_params(langmod_vocab, config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/1_model.hdf5')
                ...
                with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_selected_test.txt', 'w', encoding='utf-8') as f:
                    print(*selected_test_sents.individual_indexes, sep='\n', file=f)
                
                capgen_vocab.save_vocab(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_vocab.json')
                ...
                with model_neural_trad.TradNeuralModel(
                    ...
                    capgen_fit_stats = model.fit(dataset, config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_model.hdf5', max_batch_size=config.val_batch_size, minibatch_size=capgen_minibatch_size, max_epochs=config.max_epochs, early_stop_patience=config.early_stop_patience, listener=_FitListener())
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_train.txt', 'w', encoding='utf-8') as f:
                        print('epoch', 'train logpplx', 'val logpplx', sep='\t', file=f)
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_probs.txt', 'w', encoding='utf-8') as f:
                        for logprobs in tokens_logprobs:
                            print(*logprobs, sep='\t', file=f)
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_sents.txt', 'w', encoding='utf-8') as f:
                        for sent in text_sents:
                            print(*sent, sep=' ', file=f)
                    print()
                    print()
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_wmd.txt', 'w', encoding='utf-8') as f:
                    ...
                    with open(config.results_dir+'/partialtraining/'+corpus+'/'+dir_name+'/2_rets.txt', 'w', encoding='utf-8') as f:
                ...
                with open(config.results_dir+'/partialtraining/'+corpus+'/results2_noearlystop.txt', 'a', encoding='utf-8') as f:
                    print(
                        corpus,
                        frozen_prefix,
                        max_epochs,
