...
already_seen = set()
with open(config.results_dir+'/randomrnn/'+corpus+'/results.txt', 'r', encoding='utf-8') as f:
...
with open(config.hyperpar_dir+'/randomrnn/'+corpus+'/1_best.txt', 'r', encoding='utf-8') as f:
...
with open(config.hyperpar_dir+'/randomrnn/'+corpus+'/2_best.txt', 'r', encoding='utf-8') as f:
    hyperpars = dict(line.split('\t') for line in f.read().strip().split('\n'))
...
for frozen_prefix in [True]:
    given_up = False
    for max_epochs in range(0, 0+1):
        for run in range(1, config.num_runs+1):
            ...
            lib.create_dir(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name)
            ...
            with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_corpus_indexes.txt', 'w', encoding='utf-8') as f:
                print(*dataset.train.individual_indexes, sep='\n', file=f)
            
            with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_selected_test.txt', 'w', encoding='utf-8') as f:
                print(*selected_test_sents.individual_indexes, sep='\n', file=f)
            
            langmod_vocab.save_vocab(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_vocab.json')
            ...
            with model_neural_trad.TradNeuralModel(
                ...
                while True:
                    attempts += 1
                    model.init_params()
                    langmod_fit_stats = model.fit(dataset, config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_model.hdf5', max_batch_size=config.val_batch_size, minibatch_size=langmod_minibatch_size, max_epochs=max_epochs, early_stop_patience=config.early_stop_patience, listener=_FitListener())
                    if langmod_fit_stats['stopped_early'] == False:
                        break
                    elif attempts >= config.randomrnn_max_attempts:
                        print('Finished early too many times. Giving up.')
                        print()
                        given_up = True
                        break                            
                    else:
                        print('Finished early. Retrying. Attempts so far: {}.'.format(attempts))
                        print()
                if given_up == True:
                    break
                    
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_train.txt', 'w', encoding='utf-8') as f:
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_probs.txt', 'w', encoding='utf-8') as f:
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_sents.txt', 'w', encoding='utf-8') as f:
            ...
            prefix_params = model_neural_trad.TradNeuralModel.get_saved_prefix_params(langmod_vocab, config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/1_model.hdf5')
            ...
            with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_selected_test.txt', 'w', encoding='utf-8') as f:
                print(*selected_test_sents.individual_indexes, sep='\n', file=f)
            
            capgen_vocab.save_vocab(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_vocab.json')
            ...
            with model_neural_trad.TradNeuralModel(
                ...
                capgen_fit_stats = model.fit(dataset, config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_model.hdf5', max_batch_size=config.val_batch_size, minibatch_size=capgen_minibatch_size, max_epochs=config.max_epochs, early_stop_patience=config.early_stop_patience, listener=_FitListener())
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_train.txt', 'w', encoding='utf-8') as f:
                    print('epoch', 'train logpplx', 'val logpplx', sep='\t', file=f)
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_probs.txt', 'w', encoding='utf-8') as f:
                    for logprobs in tokens_logprobs:
                        print(*logprobs, sep='\t', file=f)
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_sents.txt', 'w', encoding='utf-8') as f:
                    for sent in text_sents:
                        print(*sent, sep=' ', file=f)
                print()
                print()
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_wmd.txt', 'w', encoding='utf-8') as f:
                ...
                with open(config.results_dir+'/randomrnn/'+corpus+'/'+dir_name+'/2_rets.txt', 'w', encoding='utf-8') as f:
            ...
            with open(config.results_dir+'/randomrnn/'+corpus+'/results.txt', 'a', encoding='utf-8') as f:
                print(
                    corpus,
                    frozen_prefix,
                    max_epochs,
